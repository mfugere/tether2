...
level1.prototype = {
	create: function () {
		...
		player2 = this.add.sprite(128, 128, "player2");
		player2.animations.add("idle-down", [ 1 ], 0, true);
		player2.animations.add("walk-down", [ 0, 1, 2, 3 ], 4, true);
		player2.animations.add("idle-up", [ 5 ], 0, true);
		player2.animations.add("walk-up", [ 4, 5, 6, 7 ], 4, true);
		player2.animations.add("idle-right", [ 9 ], 0, true);
		player2.animations.add("walk-right", [ 8, 9, 10, 11 ], 4, true);
		player2.animations.add("idle-left", [ 13 ], 0, true);
		player2.animations.add("walk-left", [ 12, 13, 14, 15 ], 4, true);
		...
		this.physics.enable([ player1, player2, blocks, holes, locks ], Phaser.Physics.ARCADE);
		...
		player2.animations.play("idle-down");

		keys = this.input.keyboard.createCursorKeys();
		keys.w = this.input.keyboard.addKey(Phaser.KeyCode["W"]);
		keys.a = this.input.keyboard.addKey(Phaser.KeyCode["A"]);
		keys.s = this.input.keyboard.addKey(Phaser.KeyCode["S"]);
		keys.d = this.input.keyboard.addKey(Phaser.KeyCode["D"]);
	},
	update: function () {
		var curDir1 = player1.animations.currentAnim.name.split("-")[1];
		var curDir2 = player2.animations.currentAnim.name.split("-")[1];
		var physics = this.physics.arcade;
		physics.collide(player1, layer);
		physics.collide(player1, blocks, moveBlock, null, this);
		physics.collide(player2, layer);
		physics.collide(player2, blocks, moveBlock, null, this);
		...
		if (keys.left.isDown) {
			...
			player2.body.velocity.setTo(-100, 0);
			player1.body.velocity.setTo(0, 0);
			player2.animations.play("walk-left");
			player1.animations.play("idle-" + curDir1);
			...
			player2.body.velocity.setTo(0, -100);
			player1.body.velocity.setTo(0, 0);
